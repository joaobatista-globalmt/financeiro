...
function sortLink(string $coluna, string $label, string $sortAtual, string $dirAtual, array $filtros, string $alignClass = ''): string {
    ...
    $params['sort'] = $coluna;
    $params['dir']  = $novaDir;
    $url = 'contas_pagar.php?' . http_build_query($params);
    $seta = '';
    if ($sortAtual === $coluna) {
...
<div class="cards-grid">
    ...
    <div class="card card-success" onclick="abrirDrillDown('pago_mes', 'Pago no Mês')" style="cursor:pointer;" title="Clique para ver os lançamentos">
...
<table class="table">
    <thead>
        <tr>
            ...
            <th><?= sortLink('fornecedor', 'Fornecedor', $sortAtual, $dirAtual, $filtros) ?></th>
...
function abrirDrillDown(card, titulo) {
    ...
    fetch('contas_pagar.php?action=drilldown&card=' + encodeURIComponent(card))
...
function renderDrillDown(data) {
    ...
    if (qtd === 0) {
        ...
        var statusBadge = { pendente: 'warning', aprovada: 'info', paga: 'success', cancelada: 'secondary' };
        var dataCols = { 'pago_mes': 'data_pagamento' };
        var dataCol = dataCols[data.card] || 'data_vencimento';
        var valorCol = (data.card === 'pago_mes') ? 'valor_pago' : 'valor';
        html += '<table class="tabela-drill"><thead><tr>';
        html += '<th>Data</th><th>Descri\u00e7\u00e3o</th><th>Fornecedor</th><th>Categoria</th><th>N\u00ba Doc</th><th class="valor">Valor</th><th>Status</th>';
        html += '</tr></thead><tbody>';
        for (var i = 0; i < data.contas.length; i++) {
            ...
            var forn = (c.fornecedor_nome || '').replace(/</g, '&lt;').replace(/>/g, '&gt;');
