...
$titulo = $dados['titulo'] ?? 'Por Cliente';
$headers = $dados['headers'] ?? [];
$rows = $dados['rows'] ?? [];
$grupos = $dados['grupos'] ?? [];

// Como este relatorio eh SOMA das contas a receber por cliente
$valorReceber = 0.0;
$qtdReceber = 0;
$valorPagar = 0.0;
$qtdPagar = 0;
foreach ($grupos as $g) {
    $valorReceber += (float)($g['valor'] ?? 0);
    $qtdReceber   += (int)($g['qtd'] ?? 0);
}
...
<div class="page-header">
    ...
    <div>
        <a href="relatorio_exportar.php?tipo=cliente&formato=csv&data_inicio=<?= urlencode($dataInicio) ?>&data_fim=<?= urlencode($dataFim) ?>&status=<?= urlencode(implode(',', $statusFiltro)) ?>&nome=<?= urlencode($nomeFiltro) ?>" class="btn">📥 CSV</a>
        <a href="relatorio_exportar.php?tipo=cliente&formato=pdf&data_inicio=<?= urlencode($dataInicio) ?>&data_fim=<?= urlencode($dataFim) ?>&status=<?= urlencode(implode(',', $statusFiltro)) ?>&nome=<?= urlencode($nomeFiltro) ?>" class="btn" target="_blank">📄 PDF</a>
...
<form method="get" class="filters-bar">
    ...
    <a href="relatorio_show.php?tipo=cliente" class="btn">Limpar</a>
</form>

<?php if (!$temRegistros): ?>
    <div class="alert alert-info" style="margin: 20px 0;">Nenhum cliente encontrado no período selecionado.</div>
<?php else: ?>
    ...
    <div class="report-cards-row" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 16px;">
        <div class="report-total-card" style="background: #f9fafb; border-left: 4px solid #9ca3af; padding: 14px 16px; border-radius: 6px;">
            ...
            <div style="font-size: 12px; color: #6b7280; margin-top: 2px;">n/a (relatório de clientes)</div>
        </div>
        <div class="report-total-card" style="background: #f0fdf4; border-left: 4px solid #16a34a; padding: 14px 16px; border-radius: 6px;">
            <div style="font-size: 12px; color: #166534; text-transform: uppercase; font-weight: 600;">🟢 A Receber (clientes)</div>
            ...
            <div style="font-size: 12px; color: #6b7280; margin-top: 2px;">
                <?= $qtdReceber ?> conta(s) &middot; <?= count($grupos) ?> cliente(s)
            </div>
        </div>
    ...
    <!-- Ranking de clientes -->
    <h2 style="margin: 8px 0 12px 0; font-size: 16px; color: #111827; font-weight: 700;">📊 Ranking de Clientes</h2>
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 12px; margin-bottom: 24px;">
        <?php foreach ($grupos as $g): ?>
            <div style="background: #fff; border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px 14px;">
                ...
                <p style="margin: 2px 0; font-size: 12px; color: #6b7280;">CPF/CNPJ: <?= htmlspecialchars($g['doc'] ?? '—') ?></p>
                ...
                <p style="margin: 2px 0; font-size: 12px; color: #16a34a;">Recebido: R$ <?= number_format((float)($g['recebido'] ?? 0), 2, ',', '.') ?></p>
    ...
    <table class="table">
        ...
        <tfoot>
            <tr class="report-total-row" style="background: #1e40af; color: white;">
                <th colspan="4" style="text-align: right; padding: 10px 12px; font-size: 14px;">TOTAL GERAL (<?= count($grupos) ?> clientes):</th>
                <th style="text-align: right; padding: 10px 12px; font-size: 14px; font-variant-numeric: tabular-nums;">
                    R$ <?= number_format($valorReceber, 2, ',', '.') ?>
                </th>
                <th style="text-align: right; padding: 10px 12px; font-size: 14px; font-variant-numeric: tabular-nums;">
                    R$ <?= array_sum(array_column($grupos, 'recebido')) > 0 ? number_format(array_sum(array_map(fn($g) => (float)($g['recebido'] ?? 0), $grupos)), 2, ',', '.') : '0,00' ?>
                </th>
                <th style="padding: 10px 12px; font-size: 12px;"><?= $qtdReceber ?> contas</th>
            </tr>
        </tfoot>
    </table>
<?php endif; ?>
