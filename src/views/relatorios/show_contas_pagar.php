...
// Como este relatorio eh SOMENTE contas a pagar, o card "A Receber" fica zerado
$valorPagar = (float)($totais['valor'] ?? 0);
$qtdPagar = (int)($totais['qtd'] ?? 0);
$valorReceber = 0.0;
$qtdReceber = 0;
$saldo = $valorReceber - $valorPagar; // sempre negativo aqui
$temRegistros = $qtdPagar > 0;
?>
<div class="page-header">
    <h1><?= htmlspecialchars($dados['titulo'] ?? 'Contas a Pagar') ?></h1>
    <div>
        <a href="relatorio_exportar.php?tipo=contas_pagar&formato=csv&data_inicio=<?= urlencode($dataInicio) ?>&data_fim=<?= urlencode($dataFim) ?>&status=<?= urlencode(implode(',', $statusFiltro)) ?>" class="btn">📥 CSV</a>
        <a href="relatorio_exportar.php?tipo=contas_pagar&formato=pdf&data_inicio=<?= urlencode($dataInicio) ?>&data_fim=<?= urlencode($dataFim) ?>&status=<?= urlencode(implode(',', $statusFiltro)) ?>" class="btn" target="_blank">📄 PDF</a>
        <a href="relatorios.php" class="btn">← Voltar</a>
    </div>
</div>

<form method="get" class="filters-bar">
    <input type="hidden" name="tipo" value="contas_pagar">
    ...
    <a href="relatorio_show.php?tipo=contas_pagar" class="btn">Limpar</a>
</form>

<?php if (!$temRegistros): ?>
    <div class="alert alert-info" style="margin: 20px 0;">Nenhuma conta a pagar encontrada no período selecionado.</div>
<?php else: ?>
    ...
    <div class="report-cards-row" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px; margin-bottom: 16px;">
        <div class="report-total-card" style="background: #fef2f2; border-left: 4px solid #dc2626; padding: 14px 16px; border-radius: 6px;">
            ...
            <div style="font-size: 12px; color: #6b7280; margin-top: 2px;">
                <?= $qtdPagar ?> conta(s) &middot;
                Pago: R$ <?= number_format((float)($totais['pago'] ?? 0), 2, ',', '.') ?>
            </div>
        </div>
    ...
    <table class="table">
        ...
        <tfoot>
            <tr class="report-total-row" style="background: #1e40af; color: white;">
                ...
                <th style="text-align: right; padding: 10px 12px; font-size: 14px; font-variant-numeric: tabular-nums;">
                    R$ <?= number_format((float)($totais['pago'] ?? 0), 2, ',', '.') ?>
                </th>
